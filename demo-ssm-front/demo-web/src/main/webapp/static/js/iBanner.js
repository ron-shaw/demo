        ...
        a.updateActiveSlide = function(c) {
            if (a.initialized && 0 != a.slides.length) {
                ...
                if (a.slides[a.activeIndex]) {
                    ...
                    var aSlideObject = $(a.slides[a.activeIndex]).find(".carlist_img");
                    if (aSlideObject.attr("data-banner") && $(window).scrollTop() < $('#banner').offset().top+$('#banner').height()) {
                        aSlideObject.attr("src", $(a.slides[a.activeIndex]).find(".carlist_img").attr("data-banner"));
                        aSlideObject.load(function(){
                            aSlideObject.prev('.sold').show();
                        });
                    };
                }
            }
            ...
            if (b.pagination && !(1 > a.slides.length)) {
                ...
                if (d && (d = a.paginationButtons, 0 != d.length)) {
                    ...
                    if (b.paginationAsRange) {
                        ...
                        b.loop ? d[a.activeLoopIndex].className += " " + b.paginationActiveClass :d[a.activeIndex].className += " " + b.paginationActiveClass;numchange.text(a.activeLoopIndex+1);
...
var newSlideSize = function slideSize(){
    var iw = Math.ceil($(".ibanner").width()/2);
    var sw = Math.ceil($(".sbanner").width()/1.64);
    var dw = Math.ceil($(".dbanner").width()/3.8);
    $(".ibanner,.banner-wrapper,.slide").height(iw);
    $(".sbanner,.banner-wrapper,.slide").height(sw);
    $(".dbanner,.banner-wrapper,.slide").height(dw);
};
newSlideSize();
$(window).resize(function(){
    newSlideSize();
});
var numchange = $("#numBer1");
var focusNumTop = $("#banner a").size();
$("#numBer2").text(focusNumTop);
$(function(){
